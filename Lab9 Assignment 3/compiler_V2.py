...
class Scanner:
    ...
    def consume(self, *expected_tokens):
        ...
        if prev_token[0] in expected_tokens:
            self.current_token = self.get_token() 
            if (prev_token[0] == 'NUM') or (prev_token[0] == 'ID'):
                return prev_token
            else:
                return prev_token[0]
        else:
            self.unexpected_token(prev_token[0], expected_tokens)
...
class BooleanExpression_AST:
    ...
    def false_code(self, label):
        l1 = label_generator.next()
        return self.left.true_code(l1) + self.right.false_code(label) + l1 + ':\n'
...
class BooleanTerm_AST:
    ...
    def true_code(self, label):
        l1 = label_generator.next()
        return self.left.false_code(l1) + self.right.true_code(label) + l1 + ':\n'
    def false_code(self, label):
...
class BooleanFactor_AST:
    ...
    def indented(self, level):
        return indent('not', level) + \
            self.comparison.indented(level+1)    
    def true_code(self, label):
...
def boolean_expression():
    ...
    while scanner.lookahead() == Token.OR:
        or_token = scanner.consume(Token.OR)
        tree = boolean_term()
        result = BooleanExpression_AST(result, or_token, tree)
    return result

def boolean_term():
    result = boolean_factor()
    while scanner.lookahead() == Token.AND:
        and_token = scanner.consume(Token.AND)
        tree = boolean_factor()
        result = BooleanTerm_AST(result, and_token, tree)
    return result
...
def write():
    ...
    to_write = expression()
    return Write_AST(to_write)

def read():
    scanner.consume(Token.READ)
    to_read = identifier()
    return Read_AST(to_read)
