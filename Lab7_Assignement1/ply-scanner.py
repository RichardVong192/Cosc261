...
Regular expressions were used in the superqu iz, but if anyone needs
...
reserved = {
    ...
    'then': 'THEN',
    'while': 'WHILE',
    'read': 'READ',    #add reserved word 'read'
    'write': 'WRITE'   #add reserved word 'write'
}
...
def t_ID(t):
    ...
    if t.value in reserved:         #if t.value is in the list of reserved words
        t.type = reserved[t.value]  #change t.type to the reserved word
    return t
