...
class Scanner:
    ...
    def skip_white_space(self):
        ...
        while (self.current_char_index < len(self.input_string) and self.input_string[self.current_char_index].isspace()):
            #while string is not empty go through all characters at its index, if character is space:
    ...
    def get_token(self):
        ...
        if token is None and (self.current_char_index < len(self.input_string)):
            #if input contains extra non-white space char that don't match tokens
            self.no_token()
            #call no token
    ...
    def consume(self, *expected_tokens):
        ...
        if prev_token[0] in expected_tokens:  #if in expected tokens
            self.current_token = self.get_token() #consumes it
            if (prev_token[0] == 'NUM') or (prev_token[0] == 'ID'): #number or identifier
                return prev_token
            else:
                return prev_token[0]
        else:
